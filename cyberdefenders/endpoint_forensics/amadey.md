...
## Scenario
An after-hours alert from the Endpoint Detection and Response (EDR) system flags suspicious activity on a Windows workstation.<br>
The flagged malware aligns with the Amadey Trojan Stealer.<br>
...
To analyze the memory dump, we need to use volatility3.<br>
Volatility3 syntax is like<br>
```
vol.py -f dump_file plugin
```
Plugin allows to analyze specific areas.<br>
In this case, I wanted to see the parent-child relationship in Windows, so I used "windows.pstree".
```
./vol.py -f ../../Artifacts/'Windows 7 x64-Snapshot4.vmem' windows.pstree
```
There are lots of parent-child, but one parent process name was suspicious (lssass, not lsass), and spawned rundll32.exe, which seems suspicious.<br>
...
Google Searched the right plugin, and found "windows.dlllist", which lists the DLLs and full path of the main process.<br>
In my scenario, the process ID was 2748, so the syntax was like<br>
```
./vol.py -f ../../Artifacts/'Windows 7 x64-Snapshot4.vmem' windows.dlllist --pid 2748
```
It revealed several loaded modules. One of file outputs is different from others.<br>
...
Based on help, used "windows.netscan" plugin to find network connections.
```
./vol.py -f ../../Artifacts/'Windows 7 x64-Snapshot4.vmem' windows.netscan
```
Found "ForeignAddr" as a C2 server. 41[.]75[.]84[.]12 had 80 (http), which indicates active C2 communication.
...
"Memmap" allows to find all the memory pages for the specific process. It includes HTTP GET request as well.<br>
Ran the following code to dump the memory map, then search "GET /" since I wanted to find additional tools or modules with C2. <br>
```
...
Investigated the command line. The child process (rundll32.exe) had the above file as a command line argument.
```
...
It was a very straightforward question. Based on the above, rundll32.exe was initiated by lssass.exe.<br>
...
Investigated file system to find persistence.<br>
Used "filescan".
```
./vol.py -f ../../Artifacts/'Windows 7 x64-Snapshot4.vmem' windows.filescan | grep .exe
```
lssass.exe was under Tasks directory. It indicated the scheduled task, common persistence technique.<br>
